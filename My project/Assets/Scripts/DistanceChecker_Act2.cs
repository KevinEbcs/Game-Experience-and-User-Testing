...
public class DistanceChecker_Act2 : MonoBehaviour
{
    ...
    void Update()
    {
        if (!fifth_text_active && !text_active)
        {
        ...
        } else if (!sixth_text_active && !text_active && fifth_text_active && Vector3.Distance(Crystal.transform.position, new Vector3(-1.16999996f,2.1400001f,11.3500004f)) < 1 && DistanceChecker.GetDistance(Player, Crystal) < 4)
        {
            text.text = "Enchanted by the sounds, you creep deeper into the maze.";
            Time.timeScale = 0f; // freeze time
            sixth_text_active = true;
            text_active = true;
        } else if (!seventh_text_active && !text_active && sixth_text_active && Vector3.Distance(Crystal.transform.position, new Vector3(13.2299995f,1.13999999f,8.18000031f)) < 1 && DistanceChecker.GetDistance(Player, Crystal) < 5)
        {
            text.text = "It seems as if the crystal has appeared again.";
            Time.timeScale = 0f; // freeze time
            seventh_text_active = true;
            text_active = true;
        } else if (!eighth_text_active && !text_active && seventh_text_active && Vector3.Distance(Crystal.transform.position, new Vector3(13.2299995f,1.13999999f,8.18000031f)) < 1 && DistanceChecker.GetDistance(Player, Crystal) < 3)
        {
